...
func zone01Levels() []Level {
	specs := []practiceSpec{
		...
		zonePrintSpec(34, "Search Replace", "Strings · replacement", "Checkpoint 20%",
			"SearchReplace(value string, old string, replacement string) string",
			"Replace every exact occurrence of one character.",
			"A string plus one-character old and replacement strings.", "The replaced string followed by newline; unchanged when old is absent.",
		...
		zonePrintSpec(53, "Word Match", "Strings · subsequences", "Checkpoint 50%",
			"WdMatch(needle string, haystack string) string",
			"Print a word only when it can be formed in order from another string.",
			"A needle and a haystack.", "needle followed by newline when it is a subsequence; otherwise no output.",
